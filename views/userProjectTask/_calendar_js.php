...
if( $showExtendedFields ) {
  $show_ext_field_str = 'jQuery("#UserProjectTask_status").val(calEvent.status);
...
<?php $this->registerJs( '  
    ...
    function submitTaskForm(isClone) {
        if(isClone) {
            ...
            submitAJAXRequest( false, "'.Yii::$app->urlManager->createUrl( 'userProjectTask/saveTask' ).'",
    ...
    function submitAutomaticTaskForm(calEventId, calEventDateIni, calEventDateFin) {
        ...
        submitAutomaticAJAXRequest( false, "'.Yii::$app->urlManager->createUrl( 'userProjectTask/automaticSaveTask' ).'",
            updatedTask );
    }

    function deleteTask(taskId){
        submitAJAXRequest( true, "'. Yii::$app->urlManager->createUrl( 'userProjectTask/delete' ).'", {id: taskId} );
    }
')
?>
<?php $this->registerJs( ' 
    ...
    function setTaskFormInitValues(calEvent){
        ...
        '.$show_ext_field_str.'
            ...
            if( calEvent.readonly ){
                ...
                var url = "'.Yii::$app->urlManager->createUrl( "AJAX/retrieveProjectsFromCustomerIdAsListOptions" ) .'";
                var data = {
    ...
    jQuery(document).ready(function(){
        ...
        var showDate = '.$dateToLoad .';
        calendar.weekCalendar({
            data: function(start,end,callback){
                calendar.weekCalendar("clear");
                jQuery.getJSON("'. Yii::$app->urlManager->createUrl( "AJAX/retrieveTasksForUserAndWeek" ) .'",
					{
					...
					function(result){
                        ...
                        '.(!$isWorker ? 
                              '
                                    ...
                                    $.get("'.Yii::$app->urlManager->createUrl( "AJAX/retrieveWorkers" ).'",
                                                {
                       ...
                       var selectedCompany = jQuery("#company_selector").val();
                            $.get("'.Yii::$app->urlManager->createUrl( "AJAX/retrieveCompanies" ).'",
                                        {
						startFilter: start.toString("dd/MM/yyyy"),
                                                endFilter: end.toString("dd/MM/yyyy"),
                                                selectWorkersPrompt: "Todos", 
...
'.
    (! $isWorker ? ('var userSelector = jQuery("#user_selector");
				...
				if( companySelector.length == 0 ) {
					var companies = "'.str_replace('"',"'",preg_replace( "/\n+/", "", Html::dropDownList("company_selector", "", ArrayHelper::map( $customers, "id", "name" ), array(
                                "prompt" => "Todos",
                                "onchange" => "updateTasks()",
                                "style" => "font-size: 1.2em",
                                "id" => "company_selector",
